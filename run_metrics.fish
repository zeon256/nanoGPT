...
function run_training_iteration
    ...
    set strace_output_file "./$run_folder/$benchmark/run_$run_number.strace.json"
    set perf_output_file "./$run_folder/$benchmark/run_$run_number.perf.json"

    log_message "Starting run number $run_number"

    strace -c -e brk,mmap,munmap uv run $train_script config/train_shakespeare_char.py --max_iters=$num_epoch --results_path=$json_output_file 2>$output_file
    log_message "Procesing strace output"

    # Process the strace output 
    uv run strace_parse.py --input_file $output_file --output_file $strace_output_file

    perf stat -e dTLB-loads,dTLB-load-misses,page-faults uv run $train_script config/train_shakespeare_char.py --max_iters=$num_epoch --results_path=$json_output_file 2>>$output_file
    log_message "Processing perf output"

    # Process the perf output 
    uv run perf_parse.py $output_file $perf_output_file
...
function main
    ...
    log_message "Generating tables"

    uv run generate_tables.py $run_folder tables.txt
